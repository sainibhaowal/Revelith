...
import { generateTocFieldXml, type TocEntry } from '@revelith/docx-engine'
...
export function normalizeCommandObj(cmd: Record<string, unknown>): Record<string, unknown> {
  ...
  const validKey = keys.find((k) => COMMAND_NAMES.includes(k as CommandName))
  if (validKey) {
    return { [validKey]: cmd[validKey] }
  }
  return cmd
}
...
export function validateEnvelope(envelope: unknown): string | null {
  ...
  for (let i = 0; i < commands.length; i++) {
    let command = commands[i] as Record<string, unknown>
    if (!command || typeof command !== 'object') return `command #${i}: must be an object`
    command = normalizeCommandObj(command)
    commands[i] = command as any
