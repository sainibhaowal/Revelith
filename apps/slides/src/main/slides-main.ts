/**
 * Revelith Slides main process : pptx parsing/render-tree building/edit application/saving all live
...
import { gskApiKey, gskSlideGenerate, setGskProxyUrl } from '@revelith/ai-search'
import {
  appMenuLabels,
...
import { getUiLang, normalizeLang, setUiLang } from '@revelith/i18n'
...
import { buildRenderSlide, EMU_PER_PX_96, type RenderSlide } from '@revelith/pptx-render'
...
/**
 * Recovery drafts for never-saved decks (wcId → visible path in <Documents>/Revelith):
...
/** Directory where AI-generated drafts are saved: the configurable default save folder (falls back to <Documents>/Revelith) */
...
/**
 * Auto-save the draft to <Documents>/Revelith/<name>.pptx after AI generation completes.
...
export function registerSlidesIpc(): void {
  ...
  // converting. Enabled when gsk is logged in; REVELITH_CLOUD_SLIDE=0 is the kill switch.
  const cloudSlideEnabled = () => process.env.REVELITH_CLOUD_SLIDE !== '0' && !!gskApiKey()
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        // ultra = opus-class model, matching the local path's quality tier; REVELITH_CLOUD_SLIDE_TIER=standard opts down
        const tier = process.env.REVELITH_CLOUD_SLIDE_TIER === 'standard' ? 'standard' : 'ultra'
        ...
        const dir = join(app.getPath('temp'), 'revelith-cloud-pages')
  ...
  ipcMain.handle('slides:copy-slide', (e, slideIndex: number, pngBase64?: string) => {
    ...
    clipboard.writeBuffer('io.revelith.slides.slide', Buffer.from('1'))
    return true
  })
  ...
  ipcMain.handle('slides:clipboard-external', () => {
    ...
    if (slideClipboard && marker('io.revelith.slides.slide')) return { kind: 'slide' }
    if (marker('io.revelith.slides.elements')) return { kind: 'internal' }
...
export function startSlidesStandalone(): void {
  ...
  // REVELITH_USER_DATA: test drivers point this at a scratch dir so automated
  ...
  if (!app.isPackaged && process.env.REVELITH_USER_DATA) {
    app.setPath('userData', process.env.REVELITH_USER_DATA)
  }
  ...
  app.whenReady().then(async () => {
    setUiLang(normalizeLang(process.env.REVELITH_LANG ?? app.getLocale()))
    registerSlidesIpc()
    registerAiIpc()
    registerProjectIpc()
    Menu.setApplicationMenu(buildSlidesMenu())
