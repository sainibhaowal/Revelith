...
import { revelithApiKey } from './revelith-auth'
 ...
 * Priority: GSK_API_KEY env → Revelith's own key (bills to us via its
...
async function toolCliPost(
  ...
  try {
    const resp = await fetch(`${GSK_TOOL_CLI_BASE}${path}`, {
      method: 'POST',
      // X-Agent-Type splits Revelith usage out of the proxy's "Claw" billing bucket
      headers: {
        'X-Api-Key': key,
