...
import { openPptx, savePptx, createBlankPptx, addElement, addPicture, mergeSlideFromPptx } from '../src/index'
...
async function onePagePptx(text: string, withImage = false): Promise<Uint8Array> {
  ...
  if (withImage) {
    const rawPng = new Uint8Array(Buffer.from(RED_DOT, 'base64'))
    addPicture(opened, opened.deck.slides[0]!, {
      bytes: rawPng,
      ext: 'png',
      offset: { x: 914400, y: 2743200, cx: 1828800, cy: 1828800 },
    })
  }
